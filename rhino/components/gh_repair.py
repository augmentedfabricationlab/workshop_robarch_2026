...
Inputs:
    ...
    position        (float)  cut position along the axis [m from beam start]
...
if run and box is not None and key:
    try:
        ...
        r = kernel.build_repair(
            j, frame,
            position=float(position) if position is not None else frame["length"] / 2,
        ...
        info.append("cut @ {:.3f}   band {:.2f}..{:.2f}   side: {}".format(
            r["position_used"], r["band"][0], r["band"][1],
